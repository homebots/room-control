...
import type { StatusEntry, Device, LightDevice, SensorDevice } from './types.js';

let credentials: Record<string, string> = {};

export function useConnector() {
  const readStateField = (state, field) => state.find((s) => s.code === field)?.value;

  const actions = {
    async getState(device: Partial<Device>) {
      const state: StatusEntry[] = await tuya.status('', { deviceId: device.id }, credentials).catch(() => []);
      ...
      switch (device.type) {
        ...
        case 'sensor':
          return <SensorDevice>{
            ...device,
            temperature: Number(readStateField(state, 'va_temperature')) / 10,
            humidity: Number(readStateField(state, 'va_humidity')),
          };
        default:
          return device;
      }
